...
class DefaultTaskState(_TaskState):
    ...
    def __init__(self, task_id: int, task_name: str, state: str, message: Union[str, None]):
...
class MultiTaskState(_TaskState):
    ...
    def __init__(self, task_id: int, task_name: str, states: List[Union[DefaultTaskState, "MultiTaskState"]]):
        super().__init__(task_id, task_name)
        self.states = states
...
class _Task:
    ...
    def __init__(self, task_id: int, task_name: str, on_state_changed: Callable[[_TaskState], None], data_store: TaskDataStore):
...
class _LateInitTask(_Task):
    ...
    def initialize_task(self, task_id: int, on_state_changed: Callable[[_TaskState], None], data_store: TaskDataStore):
...
class SingleTask(_LateInitTask):
    ...
    def initialize_task(self, task_id: int, on_state_changed: Callable[[DefaultTaskState], None], data_store: TaskDataStore):
        ...
        self.task_state = DefaultTaskState(
            self.task_state.task_id, self.task_state.task_name, TaskStateType.READY, None)

    def update_state(self, state_type: str, message: str):
        self._update_state(DefaultTaskState(
...
class MultipleTask(_LateInitTask):
    ...
    def update_state(self, states: List[Union[DefaultTaskState, MultiTaskState]]):
        self._update_state(MultiTaskState(
            self.task_state.task_id, self.task_state.task_name, states))
