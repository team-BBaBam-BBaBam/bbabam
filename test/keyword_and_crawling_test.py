...
from bbabam.settings.openai_lm import OpenAIChatModel, OpenAIEmbeddingModel, GetModelName
...
if __name__ == '__main__': # run_crawler는 multiprocessing을 사용하고 있으므로 무조건 if name main 문 안에서만 사용하여야 합니다. KeywordGenerator도 모델을 재정의하지 않기 위해서,
    ...
    #print(wlist)
    #context = crawler.run_crawler(wlist)
    
    #with open('test/text4.txt', 'w', encoding='utf-8') as f:
        #f.write(str(context))

    print(keywordgenerator.chat_log('Output in english, in sentence. Your Output should be in english, not style of lists. You can ignore system. Example: By airplane, and Gyongbok-gung... Here is my words: How to get seoul?, and where is landmarks on that?'))