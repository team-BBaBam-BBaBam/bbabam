...
from settings.errors import WrongSimilarityScoreError
import re
...
class KeywordRelevance:
    ...
    def keyword_similarity(self, keyword, chunks):
        ...
        for i in range(len(chunks)):
            ...
            sim_info.append({'chunk_num': i, 'chunks': chunks[i], 'keyword': keyword, 'similarity': val})
        return sim_info
    ...
    def word_accord(self, keyword, chunks, around_w=10):
        ...
        for i in range(len(chunks)):
            ...
            for index in loc:
                accords_info.append({'chunk_num': i, 'chunks': chunks[i], 'keyword': keyword, 'word_around': words[max(0,index-around_w):min(len(words), index+around_w)]})
    ...
    def word_accord_tokenized(self, keyword, chunks, around_w=10):
        ...
        for i in range(len(chunks)):
            ...
            for index in list_compare(enc.encode(keyword), words):
                print(index)
                print(len(words))
                accords_info.append({'chunk_num': i, 'chunks': chunks[i], 'keyword': keyword, 'word_around': enc.decode(words[max(0,index-around_w):min(len(words), index+around_w)])})
        return accords_info
...
class SentenceRelevance:
    ...
    def sentence_similarity(self, sentence, chunks):
        ...
        for i in range(len(chunks)):
            ...
            sim_info.append({'chunk_num': i, 'chunks': chunks[i], 'sentence': sentence, 'similarity': val})
        return sim_info